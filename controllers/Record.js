...
function selectActionByChatroomTag(Tag,eid,nextstep){
  ...
  const connection = new sql('CALS');
  console.log("chatroomTag",Tag);
  var querytext = "SELECT * FROM `behaviorRcord`,`tester` WHERE `tester`.`chatroomTag`='"+Tag+"' AND `tester`.`eid` ='"+eid+"' AND `behaviorRcord`.`tid`=`tester`.`tid` ORDER BY `behaviorRcord`.`time` ASC";
  console.log(querytext);
